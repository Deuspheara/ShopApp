 ...
 * _Eshopapp_
 *
 * fr.deuspheara.eshopapp.data.network.paging.ApiPagingSource
 *
 * ### Information
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, T> {
        return try {
            ...
            if (!response.isSuccessful) throw ApiException(
                code = response.code(),
                error = response.errorBody()?.string()?.let(gson::parse)
            )
            val body = response.body() ?: throw IllegalStateException("Body is null")
            LoadResult.Page(
                body.results,
                if (pageNumber == FIRST_PAGE_INDEX) null else pageNumber - 1,
                if (body.results.isEmpty()) null else pageNumber + 1
            )
        } catch (e: Exception) {
            LoadResult.Error(e)
        }
